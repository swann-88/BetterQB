    ...
    return `
      <article id="section-trackers" class="details-card details-primary">
        ...
        <div class="details-table">
          ${trackers
            .slice(0, 40)
            .map(（跟踪器） => `<div class="details-row"><span>${esc(tracker.url || tracker.msg || "（跟踪器）")}</span><span>种子 ${esc(tracker.num_seeds)}</span><span>节点 ${esc(tracker.num_peers)}</span></div>`)
  ...
  const render节点 = (data) => {
    ...
    return `
      <article id="section-peers" class="details-card details-primary">
        <h4>节点 / connections</h4>
        <p class="section-note">${peers.length} 个节点（只读快照）。</p>
        <div class="details-table">
          ${peers
            .slice(0, 60)
            .map(（节点） => `<div class="details-row"><span>${esc(peer.ip || peer.id || "（节点）")}</span><span>${esc(peer.client || "")}</span><span>下载 ${esc(formatSpeed(peer.dl_speed || 0))} | 上传 ${esc(formatSpeed(peer.up_speed || 0))}</span></div>`)
  ...
  return `
    <div class="sheet-modal details-sheet">
      ...
      <section class="sheet-scroll details-scroll">
        ...
        `)}
        ${renderSectionState("files", "Content / files", sections.files, render文件)}
        ${renderSectionState("trackers", "跟踪器", sections.trackers, render跟踪器)}
        ${renderSectionState("peers", "节点 / connections", sections.peers, render节点)}
...
function renderSettingsContent(state) {
  ...
  const renderGroup = (name) => {
    const group = view.groups.find((item) => item.name === name);
